...
namespace App\SuperAdmin\Logics;
...
class RecruitmentLogic extends RecruitmentUseCase
{
    ...
    /*
    |--------------------------------------------------------------------------
    | Create Master Employee FIRST TIME
    |--------------------------------------------------------------------------
    */
...
//        $request->request->add(['employeeNo' => str_random(6)]);
    ...
    public function handleEmployment($request)
    {
        ...
        if ($employment->employee) {
            ...
            $this->givePermissions($employment->employee);
...
//                }
    ...
    private function givePermissions($employee)
    {
        ...
        $permissions = array(
            ...
            'edit setting',
            'view attendance',
            'access kiosk setting',
            'access salary',
            'generate attendance',
            'edit attendance',
            'access storage setting',
            'access misc options'
        );
        ...
        if ($employee->user) {
            ...
            foreach ($permissions as $permission) {
                $employee->user->givePermissionTo($permission);
            }
